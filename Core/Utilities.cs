  ...
  public static class Utilities
  {
    ...
    public static bool IsNumeric(object value)
    {
      double dbl;
			...
			bool isDouble=true;
			try {dbl=double.Parse(value.ToString(),System.Globalization.NumberStyles.Any,System.Globalization.NumberFormatInfo.InvariantInfo);}catch{isDouble=false;}
			return isDouble;
#else
      ...
      else
      {
        ...
        desiredType = Utilities.GetPropertyType(desiredType);
			...
			if (desiredType.IsEnum && (valueType.Equals(typeof(string)) || Enum.GetUnderlyingType(desiredType).Equals(valueType)))
				return System.Enum.Parse(desiredType, value.ToString(), false);
...
#else
				SmartDateConverter cnv = null; ;

				if (Utilities.GetPropertyType(desiredType).Equals(typeof(SmartDate)) || Utilities.GetPropertyType(desiredType).IsSubclassOf(typeof(SmartDate)))
					cnv = new SmartDateConverter();
#endif
